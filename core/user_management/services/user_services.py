...
from repository.user_repository import IUserRepository as UserRepository
from entity.user_entity import UserEntity
from django_backend_starter.apps.loan.models import (
    UserModel,
    AdminAudit,
    PasswordHistory,
)
...
class UserService:
    ...
    def add_agent(
        self, email, first_name, last_name, contact_number, branch, region
    ) -> UserEntity:
    ...
    def suspend_user(self, admin: UserModel, user: UserModel, reason: str):
        ...
        AdminAudit.objects.create(
            admin=admin, target_user=user, action="SUSPEND_USER", reason=reason
        )

    def reactivate_user(self, admin: UserModel, user: UserModel):
        user.reactivate()
        AdminAudit.objects.create(
            admin=admin, target_user=user, action="REACTIVATE_USER"
        )
