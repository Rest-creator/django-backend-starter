...
class CartRepository:
    def add_item(
        ...
        return CartItemEntity(
            item.id, item.user_id, item.product_id, item.quantity, item.price
        )
    ...
    def get_user_cart(self, user_id: int) -> List[CartItemEntity]:
        ...
        return [
            CartItemEntity(i.id, i.user_id, i.product_id, i.quantity, i.price)
            for i in items
        ]
...
class OrderRepository:
    def create(self, user_id: int, total_amount: float) -> OrderEntity:
        ...
        return OrderEntity(
            order.id, order.user_id, order.total_amount, order.status, order.created_at
        )
...
class PaymentRepository:
    ...
    def update_status(self, payment_id: int, status: str, transaction_ref: str = None):
        ...
        Payment.objects.filter(id=payment_id).update(
            status=status, transaction_ref=transaction_ref
        )
