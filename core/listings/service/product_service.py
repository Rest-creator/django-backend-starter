...
class ProductService:
    ...
    @staticmethod
    @transaction.atomic
    def create_product(user_id: int, payload: dict, images_files: Optional[List] = None) -> ProductEntity:
        """
    ...
    @staticmethod
    @transaction.atomic
    def update_product(product_id: int, user_id: int, payload: dict, images_files: Optional[List] = None) -> ProductEntity:
        ...
        allowed = ["name","quantity","unit","price","category","description","organic","location","status"]
    ...
    def list_products(user_id: Optional[int] = None):
        qs = ProductRepository.list_by_user(user_id) if user_id else ProductRepository.list_all()
    ...
    def _upload_to_bytescale(images_files) -> List[str]:
        ...
        for f in images_files:
            f.seek(0)
            content = f.read()
            url = BytescaleClient.upload_file(f.name, content, getattr(f, "content_type", "application/octet-stream"))
            urls.append(url)
        return urls
