...
class ProductRepository:
    ...
    def list_all() -> QuerySet:
        return Product.objects.all().select_related("user").prefetch_related("product_images")

    @staticmethod
    def list_by_user(user_id: int) -> QuerySet:
        return Product.objects.filter(user_id=user_id).select_related("user").prefetch_related("product_images")
