...
class IsAdminReviewer(BasePermission):
    def has_permission(self, request, view):
        return bool(request.user and request.user.is_authenticated and (request.user.is_staff or getattr(request.user, "is_admin_reviewer", False)))
...
class IsOwnerOrAdmin(BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        if request.method in SAFE_METHODS:
            return True
        return (getattr(request.user, "is_staff", False)) or (obj.created_by_id == request.user.id)
