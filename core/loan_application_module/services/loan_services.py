...
class LoanService:
    ...
    def submit_application(self, *, agent_id: int, client_id: int, amount_requested: float, term_months: int, notes: str = "") -> LoanApplicationEntity:
    ...
    def decide(self, *, loan_id: int, admin_id: int, approve: bool) -> Optional[LoanApplicationEntity]:
        new_status = LoanApplication.STATUS_APPROVED if approve else LoanApplication.STATUS_REJECTED
        entity = self.loan_repo.update_status(loan_id, new_status, reviewed_by_id=admin_id)
