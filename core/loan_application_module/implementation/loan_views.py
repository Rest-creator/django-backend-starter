...
from ..serializers.loan_serializer import LoanCreateSerializer, LoanDetailSerializer, LoanDecisionSerializer
from .models import LoanApplication
from .permissions import IsAgent, IsAdminReviewer
...
class LoanForAgentListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        return LoanApplication.objects.filter(submitted_by=self.request.user).select_related("client").order_by("-created_at")

    def get_serializer_class(self):
        return LoanCreateSerializer if self.request.method == "POST" else LoanDetailSerializer
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            return Response(LoanDetailSerializer(obj).data, status=status.HTTP_201_CREATED)
...
class LoanDecisionView(generics.UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        service = LoanService(ClientRepository(), LoanRepository())
        entity = service.decide(loan_id=loan_id, admin_id=request.user.id, approve=approve)
        if not entity:
...
class ClientLoanListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if client_id:
            qs = LoanApplication.objects.filter(client_id=client_id).select_related("client")
