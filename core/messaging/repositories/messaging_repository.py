...
class ConversationRepository:
    def create(
        self, product_id: int, initiator_id: int, recipient_id: int
    ) -> ConversationEntity:
        convo = Conversation.objects.create(
            product_id=product_id, initiator_id=initiator_id, recipient_id=recipient_id
        )
...
class MessageRepository:
    ...
    def get_messages(self, conversation_id: int) -> List[MessageEntity]:
        msgs = Message.objects.filter(conversation_id=conversation_id).order_by(
            "created_at"
        )
        return [
            MessageEntity(
